...
const Grid = ({ endpoint }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const appdataResponse = await axios.get(`${config.apiUrl.replace(/\/$/, '')}/appdata`);
        ...
        setRows(appdata);
        const schemaForCallsPage = webform.find((page) => page.pageName === 'calls');
        setWebformSchema(schemaForCallsPage?.fields || []);

        // Validate data against the schema
        const validated = appdata.map((data) => {
          const validatedData = {};
          schemaForCallsPage?.fields.forEach((field) => {
  ...
  const handleEdit = () => {
    ...
    if (!selectedRow || !selectedRow.caller_number) {
      console.error('Selected row or caller_number is not defined');
      return;
    }
    ...
    console.log('Navigating to edit page with caller number:', selectedRow.caller_number);
    navigate(`/edit/${selectedRow.caller_number}`, { state: { rowData: selectedRow, schema: webformSchema } });
  };
  ...
  const handleDelete = () => {
    ...
    // Add your code for handling delete here
  };
          ...
          <Table>
            ...
            <TableBody>
              {validatedData.slice(startIndex, endIndex).map((row) => (
                <TableRow key={row.caller_number}>
                  {webformSchema.map((field) => (
                    <TableCell key={`${row.caller_number}-${field.fieldName}`}>
