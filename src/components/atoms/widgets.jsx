...
const AtomWidget = ({ widget, count, onHide, onDragStart, onDragEnter, onDragEnd, index }) => {
  const navigate = useNavigate();
  ...
  if (!widget) {
    console.error(`Widget is undefined for index ${index}`);
    return (
      <div style={{ width: '250px', height: 'auto', margin: '20px', float: 'left', background: '#DDE1E9 !important' }}>
        <p>Error: Widget data is missing</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div style={{ textAlign: 'end', marginBottom: '-20px', marginRight: '5px' }}>
        <button className='widget-hide-btn' style={{ position: 'relative', border: 'none', background: 'white', borderRadius: '100px', cursor: 'pointer' }} onClick={() => onHide(index)}>-</button>
      </div>
      <CreateWidgetItem style={{ background: '#DDE1E9 !important' }}>
        <Icon>
          <div style={{ background: 'white', borderRadius: '100px', padding: '20px', margin: 'auto', display: 'flex' }}>
            <img 
              style={{ height: '30px', width: 'auto', margin: 'auto', filter: 'brightness(0) saturate(100%) invert(50%) sepia(100%) saturate(500%) hue-rotate(190deg)' }} 
              src={widget.icon_url || 'default_icon_url'} 
              alt={widget.title} 
              onError={(e) => e.target.src = 'default_icon_url'} // Fallback if image fails to load
            />
          </div>
        </Icon>
        <div
        ...
        >
          <Title>{widget.title}</Title>
          <Count>{count !== undefined ? count : 'Loading...'}</Count>
        </div>
      </CreateWidgetItem>
    </div>
  );
