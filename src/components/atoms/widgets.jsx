...
const WidgetItem = ({ widget, dashboardName }) => {
      ...
      apiRedirectEndpointFilter.forEach((filter, index) => {
        ...
        if (matchObj) {
          ...
          if (filterValue) {
            ...
            localStorage.setItem('pageNameFilter', JSON.stringify({ 'pageName': pageNameFilter }));
  ...
  return (
    <div style={{ width: '250px', height: 'auto', margin: '20px', float: 'left' }}>
      <CreateWidgetItem onClick={() => handleWidgetClick({ widget, dashboardName })}>
        <Icon>
          <div style={{ background: '#fff6', borderRadius: '100px', padding: '20px', margin: 'auto', display: 'flex' }}>
            <img style={{ height: '30px', width: 'auto', margin: 'auto', filter: 'brightness(0) invert(1)' }} src={widget.icon_url || 'default_icon_url'} alt={widget.title} />
          </div>
        ...
        <div style={{ width: '60%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
          <Title>{widget.title}</Title>
          {count === null ? <CircularProgress size={22} /> : <Count>{count}</Count>}
        </div>
      </CreateWidgetItem>
    </div>
  );
