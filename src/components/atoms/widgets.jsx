...
const retrieveWidgetCount = async (apiEndpoint, apiEndpointFilter, dashboardName) => {
  try {
    const response = await axios.post(apiEndpoint, apiEndpointFilter, {
      headers: {
        "Content-Type": "application/json",
    ...
    const data = response.data;
    return data && data.status === "success" && data.data && data.data.length > 0
...
const CreateWidget = ({ widget, dashboardName }) => {
  ...
  useEffect(() => {
    const fetchCount = async () => {
      const countData = await retrieveWidgetCount(widget.apiEndpoint, JSON.parse(widget.apiEndpointFilter), dashboardName);
      setCount(countData);
    };
    ...
    if (Array.isArray(apiRedirectEndpointFilter)) {
      apiRedirectEndpointFilter.forEach((filter) => {
        ...
        if (matchObj) {
          ...
          if (filterValue) {
            sessionStorage.setItem("widgetFilter", JSON.stringify({ [filterKey]: filterValue }));
            sessionStorage.setItem("pageNameFilter", JSON.stringify({ pageName: pageNameFilter }));
          }
        }
      });
    }
